...
kernel_Sepconv_Backward = '''
	extern "C" __global__ void kernel_Sepconv_Backward(
		...
		{
			float temp0 = VALUE_4(gradOut, intSample, imageDepthD, intY, intX);
			for(int  intFilterX = 0; intFilterX < 51; intFilterX += 1) 
			{
				float temp1 = VALUE_4(input, intSample, imageDepthD, intY + intDepthIJ, intX + intFilterX);
				float temp3 = VALUE_4(horizontal, intSample, intFilterX, intY, intX);
				singleOutV += temp0 * temp1 * temp3;		
			}
			
			for( int  intFilterY = 0; intFilterY < 51; intFilterY += 1) 
			{
			   float temp1 = VALUE_4(input, intSample, imageDepthD, intY + intFilterY , intX + intDepthIJ);
			   float temp2 = VALUE_4(vertical, intSample, intFilterY, intY, intX);
			   singleOutH += temp0 * temp1 * temp2;
			}	
		}
