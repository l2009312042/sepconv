...
arguments_strFirst = r'F:\xxx\yyy\1\1\00001.jpg'
arguments_strSecond = r'F:\xxx\yyy\1\1\00003.jpg'
arguments_strOut = r'F:\xxx\yyy\1\1\out2.jpg'
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict(torch.load(r'F:\xxx\pretrain_model\network-lf.pytorch'))
    ...
    def forward(self, tensorFirst, tensorSecond):
        ...
        tensorFirst = torch.nn.functional.pad(input=tensorFirst,
                                              pad=[int(math.floor(51 / 2.0)), int(math.floor(51 / 2.0)),
                                                   int(math.floor(51 / 2.0)), int(math.floor(51 / 2.0))],
                                              mode='replicate')
        ...
        tensorDot1 = sepconv.FunctionSepconv(tensorInput=tensorFirst,
                                             tensorVertical=self.moduleVertical1(tensorCombine),
                                             tensorHorizontal=self.moduleHorizontal1(tensorCombine))
        tensorDot2 = sepconv.FunctionSepconv(tensorInput=tensorSecond,
                                             tensorVertical=self.moduleVertical2(tensorCombine),
                                             tensorHorizontal=self.moduleHorizontal2(tensorCombine))
...
def estimate(tensorFirst, tensorSecond):
    ...
    tensorPreprocessedFirst = torch.nn.functional.pad(input=tensorPreprocessedFirst, pad=[int(math.floor(51 / 2.0)),
                                                                                          int(math.floor(
                                                                                              51 / 2.0)) + intPreprocessedWidth,
                                                                                          int(math.floor(51 / 2.0)),
                                                                                          int(math.floor(
                                                                                              51 / 2.0)) + intPreprocessedHeight],
                                                      mode='replicate')
    tensorPreprocessedSecond = torch.nn.functional.pad(input=tensorPreprocessedSecond, pad=[int(math.floor(51 / 2.0)),
                                                                                            int(math.floor(
                                                                                                51 / 2.0)) + intPreprocessedWidth,
                                                                                            int(math.floor(51 / 2.0)),
                                                                                            int(math.floor(
    ...
    return torch.nn.functional.pad(input=moduleNetwork(tensorPreprocessedFirst, tensorPreprocessedSecond),
                                   pad=[0 - int(math.floor(51 / 2.0)),
                                        0 - int(math.floor(51 / 2.0)) - intPreprocessedWidth,
                                        0 - int(math.floor(51 / 2.0)),
                                        0 - int(math.floor(51 / 2.0)) - intPreprocessedHeight], mode='replicate')[0, :,
           :, :].cpu()
...
if __name__ == '__main__':
    ##解析txt文件
    testfile =r'F:\xxx\yyy\test.txt'

    with open(testfile, 'r') as document:
        index = 0
        for line in document:
            ...
            inframe =[]
            for idx in range(2):
                picname = "%05d.jpg" % (record[idx])
                imagename = os.path.join(line[0], picname)
                dstimagename =os.path.join(line[0], "%04d.jpg" % (record[idx]))
                inframe.append(imagename)
                shutil.copyfile(imagename, dstimagename)
            picname = "%04d.jpg" % (record[1]  -1 )
            outname = os.path.join(line[0],picname)
            index += 1

            pic1 = PIL.Image.open(inframe[0])
            pic2 = PIL.Image.open(inframe[1])
            ...
            if w1 > 1280 or h1 >720 :
                ...
                nh = 720
                nw = 960
                npic1 = pic1.resize((nw,nh), PIL.Image.ANTIALIAS)
                npic2 = pic2.resize((nw, nh), PIL.Image.ANTIALIAS)
                print("resize {0}\n".format(outname))
            if iflag == 0:
                    tensorFirst = torch.FloatTensor(
                        numpy.array(pic1)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (
                                    1.0 / 255.0))
                    tensorSecond = torch.FloatTensor(
                        numpy.array(pic2)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (
                                    1.0 / 255.0))

                    tensorOutput = estimate(tensorFirst, tensorSecond)

                    PIL.Image.fromarray(
                        (tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0).astype(numpy.uint8)).save(
                        outname)
            else:
                tensorFirst = torch.FloatTensor(
                    numpy.array(npic1)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (
                            1.0 / 255.0))
                tensorSecond = torch.FloatTensor(
                    numpy.array(npic2)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (
                            1.0 / 255.0))

                tensorOutput = estimate(tensorFirst, tensorSecond)

                nout0 =  PIL.Image.fromarray((tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0).astype(numpy.uint8))
                nout = nout0.resize((w1, h1), PIL.Image.ANTIALIAS)
                nout.save(outname)
